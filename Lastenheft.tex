...
Startet das Programm im \gls{Daemon} Modus. Optional kann man die Abhöhrende Internet Protokoll Adresse (v4 oder v6) und den Port (Dezimale 16bit Ganzzahl) im (lokalen) Netzwerk und der Http  angeben.
\begin{mycode}
bbs [<--server|-s> <ipaddr>] [<--serverport|-sp> <port>]
\end{mycode}

\subsubsection{Arbeiter starten}
Startet das Programm im \gls{Daemon} Modus. Optional kann man die Internet Protokoll Adresse (v4 oder v6) bzw. den DNS Namen des Hauptservers und den Port (Dezimale 16bit Ganzzahl) im (lokalen) Netzwerk angeben.
\begin{mycode}
...
Beendet die Aufgabe mit der angegeben ID Nummer und git den exit Status der Ausgabe aus.
\begin{mycode}
bbc <--stop|-s> <id>
\end{mycode}
...
\begin{mycode}
# Holen des CentOs Beispiels von github, es reicht ein Ordner der eine DockerFile enthält als Arbeitesverzeichnis zu haben.
git clone https://github.com/balancedbanana/DockerImage.git DockerImage
cd DockerImage
# hinzufügen des Images (falls noch nicht vorhanden, bzw. wird überschrieben)
# "." ist das Aktuelle Arbeitesverzeichnis kann auch ein anderer Ordner mit einer DockerFile sein
...
echo $taskid
# Gibt "Hallo Welt" an der Konsole Aus und beendet sich, da die Aufgabe abgeschlossen ist bzw. wartet auf das beenden.
bbc --tail $taskid
\end{mycode}
...
\subitem \bftext{Erklärung} Ziel ist zu testen, ob der Server in der Lage ist Aufgaben, die zu viel Zeit benötigen, abzubrechen.
\subitem \bftext{Ablauf} Ausgegangen wird von einem funktionierenden System, das in der Lage ist Aufgaben entgegenzunehmen und zu verarbeiten und den Auftraggeber nach Abschluss einer Aufgabe zu benachrichtigen.
...
\item[T13] \bftext{Manuelles Stoppen von Aufgaben (OFA3)}
\subitem \bftext{Erklärung} Ziel ist zu testen, ob der Nutzer in der Lage ist seine Aufgaben selbst abzubrechen.
\subitem \bftext{Ablauf} Ausgegangen wird von einem funktionierenden System, das in der Lage ist Aufgaben entegenzunehmen, zu verarbeiten und den Status von Aufgaben zurückzugeben.
...
\item[T14] \bftext{Manuelle Sicherung von Aufgaben (OFA4)}
\subitem \bftext{Erklärung} Ziel ist zu testen, ob der Nutzer seine Aufgaben manuell sichern kann.
\subitem \bftext{Ablauf} Ausgegangen wird von einem System, das in der Lage ist Aufgaben entgegenzunehmen und zu bearbeiten.
