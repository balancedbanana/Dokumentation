...
\begin{minipage}[t]{\linewidth}
\item[FA33] \textbf{\gls{Priorität} (Parameter)}
\subitem \textbf{Erklärung} Beim Einreichen einer neuen Aufgabe soll der Auftraggeber (\gls{Benutzer}) in der Lage sein, seine Aufgabe mit einer \gls{Priorität} zu versehen.
\subitem \textbf{Wichtigkeit} Optional
\subitem \textbf{Voraussetzung(en)} Der Benutzer muss mit dem Befehl eine zulässige Aufgabe einreichen.
\subitem \textbf{Nachbedingung(en)}
\subsubitem \textbf{Erfolg} Die Aufgabe wird mit der gewünschten \gls{Priorität} in die Warteschlange eingereiht oder sofort ausgeführt.
\subitem \textbf{Details} Eine \gls{Priorität} stellt eine Ordnung auf allen ausstehenden Aufgaben dar. Das bedeutet, Für je zwei (aktive oder passive) Aufgaben lässt sich bestimmen, welche von beiden zuerst ausgeführt werden soll.\newline
Die \gls{Priorität} bestimmt somit die Position einer Aufgabe innerhalb der Warteschlange (FA20).\newline
Tritt der Fall ein, dass zwei Aufgaben die selbe \gls{Priorität} haben, beide in der Warteschlange liegen und nur Rechenleistung für eine der Aufgaben verfügbar ist, so ist die Ausführungsreihenfolge der Aufgaben zufällig.\newline
Aufgaben mit hohen \glspl{Priorität} können aktive Aufgaben mit stark geringerer \gls{Priorität} unterbrechen.\newline
Aufgaben, die längere Zeit in der Warteschlange verbringen, werden in ihrer \gls{Priorität} aufgewertet.\newline
Eine aktive Aufgabe wird in ihrer \gls{Priorität} aufgewertet.\newline
Die \gls{Priorität} wird auf der Kommandozeile mit "--priority" oder "-p" angegeben.\newline
Für eine \gls{Priorität} sind folgende Werte zulässig:\newline
geringste: low (l)\newline
normal: medium (m)\newline
hoch: high (h)\newline
sehr hoch: extreme (e)\newline
höchste: bananas (b)\newline
Der globale Standardwert ist festgelegt auf medium (m).
\end{minipage}
\pagebreak
